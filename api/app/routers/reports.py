...
def list_reports(db: Session = Depends(get_db)):
    return crud.report.get_all(db)


@router.post("/", response_model=ReportOut)
def create_report(report: ReportCreate, db: Session = Depends(get_db)):
    return crud.report.create(db, report)


@router.get("/{report_id}", response_model=ReportDetailOut)
def get_report(report_id: int, db: Session = Depends(get_db)):
    report = crud.report.get_full_report(db, report_id)
    if not report:
...
def update_report(report_id: int, update: ReportUpdate, db: Session = Depends(get_db)):
    return crud.report.update(db, report_id, update)


@router.delete("/{report_id}")
def delete_report(report_id: int, db: Session = Depends(get_db)):
    # TODO: Add filesystem cleanup logic
    return crud.report.delete(db, report_id)
...
def create_mapping_report(report_id: int, data: MappingReportCreate, db: Session = Depends(get_db)):
    return crud.report.create_mapping_report(db, report_id, data)
...
def update_mapping_report(report_id: int, data: MappingReportUpdate, db: Session = Depends(get_db)):
    return crud.report.update_mapping_report(db, report_id, data)
