...
def main():
    ...
    parser.add_argument('--create_coco', default=False,
                        help='Create coco annotation file with the results from the inference')
    parser.add_argument('--netfolders', default=str, nargs='+',
                        help='List of folders where checkpoints and configs are '
    ...
    parser.add_argument('--outfolders', type=str, nargs='+', default=[],
                        help='Output folder/s for the images to save (not used for single image; use out_file instead)')
    parser.add_argument('--extensions', type=str, nargs='+', default=['.jpg'],
                        help='File extensions that are searched for')
    parser.add_argument('--pattern', default='.', help='Regex Pattern for input images')
    parser.add_argument('--include_subdirs', default=False,
                        help='Searches images additionally in all subdirs of input_folder (default: False)')
    parser.add_argument('--score_thr', default=0.6, help='Threshold for BBox Detection (default: 0.5)')
    parser.add_argument('--batch_size', default=5, help='Batch size for Model (default: 5)')
    parser.add_argument('--split_images', default=False,
                        help='Split images into tiles fore more precise detection (default: False)')
    parser.add_argument('--max_splitting_steps', default=1,
                        help='Maximum number of splitting steps, splits into 4 images each time (default: 1)')
