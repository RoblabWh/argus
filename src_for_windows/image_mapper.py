...
class ImageMapper:
    def __init__(self, path_to_images, map_width_px, map_height_px, blending, optimize, statusUpdate):
    ...
    def create_flight_report(self):
        ...
        self.statusUpdate("Step 1/4: found " + str(len(image_paths)) + " images, filtering")
        ...
        filtered_images = GimbalPitchFilter(89).filter(images)
    ...
    def __create_html(self, folder, file_name, processing_time, ir_path=None, ir_html_file_name=None, ir_map_name=None):
        path = self.path_to_images
        if self.path_to_images[-1] != "/":
            path += "/"
        ...
        html_map = HTMLMap(self.map_elements, 
                           ...
                           path + folder + self.html_file_name,
                           file_name, 
        ...
        self.statusUpdate("Step 4/4: creating html file")                  
        html_map.create_html_file()
    
    def show_flight_report(self):
        webbrowser.open(os.path.abspath(str(self.path_to_images) + str(self.html_file_name)+ ".html"), new=2)
