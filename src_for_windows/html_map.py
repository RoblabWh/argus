...
class HTMLMap():
    def __init__(self, map_elements, map_height, map_width , html_file_name, map_name, area_m, altitude, processing_time, corner_gps_left_bottom, corner_gps_right_top, middle_gps, ir_path = None, ir_html_file_name=None, ir_map_name=None):
    ...
    def get_summary(self):
        ...
        try:
            weather_info_str = weathercom.getCityWeatherDetails("".join(self.location.split(",")[1:-1]))
            weather_info_lst = weather_info_str.split(",")
            #print(weather_info_lst)
            self.temperature = weather_info_lst[17].split(":")[-1]
            self.humidity = weather_info_lst[9].split(":")[-1]
            self.altimeter = weather_info_lst[2].split(":")[-1]
            self.wind_speed = weather_info_lst[22].split(":")[-1]
            self.wind_dir_compass = weather_info_lst[23].split(":")[-1]
            self.visibility = weather_info_lst[21].split(":")[-1]
            self.wind_dir_degrees = weather_info_lst[24].split(":")[-1].replace("}","").strip()
        except:
            #print(sys.exc_info())
    ...
    def write_gallery(self, file_desciptor):
        ...
        for i in range(len(self.map_elements)):
            ...
            "<div id=\"slide"+str(i+1)+"\">\n"\
            "<div class=\"img-magnifier-container\">\n"\
            "<img class=\"center-mit\" id=\""+str(i+1)+"\" src=\""+str((self.map_elements[i].get_image().get_image_path().split("/"))[-1])+"\" "\
    ...
    def write_map(self, file_desciptor):
        ...
        file_desciptor.write("<div id=\"map\"></div>\n"\
	                     ...
	                     "\n"\
							 ...
							 "  trajectory.setStyle({opacity: (value/100)});\n"\
    ...
    def write_summary(self, file_desciptor):
        ...
        "      <td class=\"tg-031e\">"+str(self.wind_speed)+str(" km/h")+" &#8776; "+str(round(int(self.wind_speed)/3.6, 1))+" m/s </td>\n"\
        "   </tr>\n"\
        "   <tr>\n"\
        "      <td class=\"tg-031e\">Wind Direction</td>\n"\
        "      <td class=\"tg-031e\">"+str(self.wind_dir_compass).replace("E","O").replace("e","o")+"&nbsp; <div style=\"transform: rotate("+str(int(self.wind_dir_degrees)+45)+"deg);display: inline-block;font-size: 40px;\">  &#9099;</div></td>\n"\
        "   </tr>\n"\
        "   <tr>\n"\
