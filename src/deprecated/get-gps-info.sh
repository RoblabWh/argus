...
if [ -d "$1" ]; then
    ...
    exiftool -q -q -if '$gpsaltitude' -fileOrder datetimeorginal -p $1/gpx.fmt -d %Y-%m-%dT%H:%M:%SZ $1 > $1/out.gpx
    rm $1/gpx.fmt
    python3 visualize_gpx.py $1
    #myvar1=$( find $1 -iwholename "*.html" )
    echo $1
    echo $2
    
    if [ $2 == 'True' ]; then
        readarray -d '' array < <( find $1 -iwholename "*.html" -print0)
        nohup firefox ${array[0]} > /dev/null 2>&1 &
    fi
fi
